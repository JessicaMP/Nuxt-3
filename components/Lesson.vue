...
<template>
  <div class="space-y-4">
    <p class="mt-0 uppercase font-bold text-slate-400 mb-1">Lesson {{ lesson.course_order + 1 }} - duration: {{ lesson.duration }}</p>
    <h2 class="my-0">{{ lesson.title }}</h2>

    <div class="flex space-x-4">
        <a v-if="lesson.sourceUrl" :href="lesson.sourceUrl" class="font-normal text-md text-gray-500">
            Download Source Code 
        </a>

        <a v-else-if="lesson.downloadUrl" :href="lesson.downloadUrl" class="font-normal text-md text-gray-500">
            Download Video
        </a>
